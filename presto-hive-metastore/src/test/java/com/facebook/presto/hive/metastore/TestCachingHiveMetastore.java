...
public class TestCachingHiveMetastore
{
    ...
    public void testGetAllDatabases()
    {
        ...
        assertEquals(metastore.getAllDatabases(HIVE_IDENTITY), ImmutableList.of(TEST_DATABASE));
    ...
    {
        assertEquals(mockClient.getAccessCount(), 0);
        assertEquals(metastore.getAllTables(HIVE_IDENTITY, TEST_DATABASE).get(), ImmutableList.of(TEST_TABLE));
        assertEquals(mockClient.getAccessCount(), 1);
        assertEquals(metastore.getAllTables(HIVE_IDENTITY, TEST_DATABASE).get(), ImmutableList.of(TEST_TABLE));
        assertEquals(mockClient.getAccessCount(), 1);
    ...
    {
        assertFalse(metastore.getAllTables(HIVE_IDENTITY, BAD_DATABASE).isPresent());
    }
    ...
    {
        assertEquals(mockClient.getAccessCount(), 0);
        assertNotNull(metastore.getTable(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE));
        assertEquals(mockClient.getAccessCount(), 1);
        assertNotNull(metastore.getTable(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE));
        assertEquals(mockClient.getAccessCount(), 1);
    ...
    {
        assertFalse(metastore.getTable(HIVE_IDENTITY, BAD_DATABASE, TEST_TABLE).isPresent());
    ...
    public void testGetPartitionNames()
    {
        ...
        assertEquals(mockClient.getAccessCount(), 0);
        assertEquals(metastore.getPartitionNames(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE).get(), expectedPartitions);
        assertEquals(mockClient.getAccessCount(), 1);
        assertEquals(metastore.getPartitionNames(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE).get(), expectedPartitions);
        assertEquals(mockClient.getAccessCount(), 1);
    ...
    {
        assertEquals(metastore.getPartitionNames(HIVE_IDENTITY, BAD_DATABASE, TEST_TABLE).get(), ImmutableList.of());
    }
    ...
    public void testGetPartitionNamesByParts()
    {
        ...
        assertEquals(mockClient.getAccessCount(), 0);
        assertEquals(metastore.getPartitionNamesByFilter(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE, ImmutableMap.of()), expectedPartitions);
        assertEquals(mockClient.getAccessCount(), 1);
        assertEquals(metastore.getPartitionNamesByFilter(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE, ImmutableMap.of()), expectedPartitions);
        assertEquals(mockClient.getAccessCount(), 1);
    ...
    public void testCachingWithPartitionVersioning()
    {
        ...
        assertEquals(mockClient.getAccessCount(), 0);
        assertEquals(partitionCachingEnabledmetastore.getPartitionNamesByFilter(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE, ImmutableMap.of()), EXPECTED_PARTITIONS);
        assertEquals(mockClient.getAccessCount(), 1);
        assertEquals(partitionCachingEnabledmetastore.getPartitionNamesByFilter(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE, ImmutableMap.of()), EXPECTED_PARTITIONS);
        ...
        assertEquals(partitionCachingEnabledmetastore.getPartitionsByNames(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE, ImmutableList.of(TEST_PARTITION1, TEST_PARTITION2)).size(), 2);
        assertEquals(mockClient.getAccessCount(), 3);

        // Now if we fetch any or both of them, they should hit the cache
        assertEquals(partitionCachingEnabledmetastore.getPartitionsByNames(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE, ImmutableList.of(TEST_PARTITION1)).size(), 1);
        assertEquals(partitionCachingEnabledmetastore.getPartitionsByNames(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE, ImmutableList.of(TEST_PARTITION2)).size(), 1);
        assertEquals(partitionCachingEnabledmetastore.getPartitionsByNames(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE, ImmutableList.of(TEST_PARTITION1, TEST_PARTITION2)).size(), 2);
        assertEquals(mockClient.getAccessCount(), 3);
    ...
    {
        assertTrue(metastore.getPartitionNamesByFilter(HIVE_IDENTITY, BAD_DATABASE, TEST_TABLE, ImmutableMap.of()).isEmpty());
    }
    ...
    {
        assertEquals(mockClient.getAccessCount(), 0);
        metastore.getTable(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE);
        assertEquals(mockClient.getAccessCount(), 1);

        // Select half of the available partitions and load them into the cache
        assertEquals(metastore.getPartitionsByNames(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE, ImmutableList.of(TEST_PARTITION1)).size(), 1);
        assertEquals(mockClient.getAccessCount(), 2);

        // Now select all of the partitions
        assertEquals(metastore.getPartitionsByNames(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE, ImmutableList.of(TEST_PARTITION1, TEST_PARTITION2)).size(), 2);
        ...
        assertEquals(metastore.getPartitionsByNames(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE, ImmutableList.of(TEST_PARTITION1)).size(), 1);
        assertEquals(metastore.getPartitionsByNames(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE, ImmutableList.of(TEST_PARTITION2)).size(), 1);
        assertEquals(metastore.getPartitionsByNames(HIVE_IDENTITY, TEST_DATABASE, TEST_TABLE, ImmutableList.of(TEST_PARTITION1, TEST_PARTITION2)).size(), 2);
        assertEquals(mockClient.getAccessCount(), 3);
    ...
    {
        Map<String, Optional<Partition>> partitionsByNames = metastore.getPartitionsByNames(HIVE_IDENTITY, BAD_DATABASE, TEST_TABLE, ImmutableList.of(TEST_PARTITION1));
    ...
    public void testNoCacheExceptions()
    {
        ...
        try {
            metastore.getAllDatabases(HIVE_IDENTITY);
        }
