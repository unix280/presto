...
public class SemiTransactionalHiveMetastore
{
            ...
            PartitionStatistics statistics)
    {
        ...
        if (oldTableAction == null) {
            ...
            tableActions.put(table.getSchemaTableName(), new Action<>(ActionType.ADD, tableAndMore, hdfsContext, metastoreContext));
            return;
        }
    ...
    public synchronized void dropTable(HdfsContext hdfsContext, String databaseName, String tableName)
    {
        ...
        if (oldTableAction == null || oldTableAction.getType() == ActionType.ALTER) {
            MetastoreContext metastoreContext = new MetastoreContext(hdfsContext.getIdentity(), hdfsContext.getQueryId().orElse(PRESTO_QUERY_ID_NAME), hdfsContext.getSession().flatMap(MetastoreUtil::getMetastoreHeaders), columnConverterProvider);
    ...
    public synchronized Map<String, Optional<Partition>> getPartitionsByNames(MetastoreContext metastoreContext, Table table, List<String> partitionNames)
    {
        ...
        Map<List<String>, Action<PartitionAndMore>> partitionActionsOfTable = partitionActions.computeIfAbsent(new SchemaTableName(table.getDatabaseName(), table.getTableName()),
                k -> new HashMap<>());
            ...
            boolean isPathValidationNeeded)
    {
        ...
        HdfsContext hdfsContext = new HdfsContext(session, databaseName, tableName, tablePath, isNewTable, isPathValidationNeeded);
        MetastoreContext metastoreContext = new MetastoreContext(session, getMetastoreHeaders(session), columnConverterProvider);
        ...
        try {
            for (Map.Entry<SchemaTableName, Action<TableAndMore>> entry : tableActions.entrySet()) {
                ...
                MetastoreContext metastoreContext = new MetastoreContext(
                        action.getHdfsContext().getIdentity(),
                        action.getHdfsContext().getQueryId().orElse(""),
                        action.getHdfsContext().getClientInfo(),
                        action.getHdfsContext().getSource(),
