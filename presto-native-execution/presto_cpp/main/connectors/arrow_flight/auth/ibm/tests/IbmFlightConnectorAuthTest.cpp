...
class IbmCpdAuthTest : public ArrowFlightConnectorAuthTestBase {
 public:
  ...
  makeSplitsWithExtraCredentials(
      const std::initializer_list<std::string>& tickets) {
    auto splits = makeSplits(tickets);
    for (auto& split : splits) {
      if (auto flightSplit =
              std::dynamic_pointer_cast<ArrowFlightSplit>(split)) {
        flightSplit->extraCredentials.insert(
            std::make_pair(
...
TEST_F(IbmCpdAuthTest, cpdAuthenticator) {
  ...
  AssertQueryBuilder(plan)
      .connectorSessionProperty(
          ...
          ibm::CpdAuthenticator::kCpdTokenKey,
          TestServerMiddlewareFactory::kAuthToken)
      .splits(makeSplitsWithExtraCredentials({"sample-data"}))
