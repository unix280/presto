...
public class QueryRewriterManager
{
            ...
            int retryCount)
    {
        ...
        ImmutableSet.Builder<String> gfsCatalogsBuilder = ImmutableSet.builder();
        if (getQueryRewriterProvider().isPresent()) {
            ...
            for (Catalog catalog : catalogNames) {
                ...
                if (connectorsList.contains(connectorType)) {
                    gfsCatalogsBuilder.add(catalog.getCatalogName());
                }
            }
            QueryRewriterInput queryRewriterInput = new QueryRewriterInput.Builder()
                    ...
                    .setGfsEnabledCatalogs(gfsCatalogsBuilder.build())
            ...
            if (!query.equals(rewrittenQuery)) {
                log.info("createQueryInternal :: QueryId [%s] - Replacing with rewritten query", queryId.getId());
            }
            return new QueryAndSessionProperties(
                    Optional.ofNullable(rewrittenQuery),
                    Optional.ofNullable(queryRewriterOutput.getOriginalQuery()),
                    queryRewriterOutput.getSessionProperties(),
                    queryRewriterOutput.getRuntimeStats(),
                    queryRewriterOutput.getUserVisibleQuery());
        }
        return new QueryAndSessionProperties(Optional.empty(),
                Optional.empty(),
                session.getSystemProperties(),
                new RuntimeStats(), query);
    }
}
