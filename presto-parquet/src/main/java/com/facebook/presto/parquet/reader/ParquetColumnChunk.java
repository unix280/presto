...
{
    //TODO : Make this max buffer size read value configurable
    //For S3 reads 8MB - 32MB fetch size is good default
    @VisibleForTesting
    public static final int MAX_BUFFER_SIZE_DATA_SOURCE_READ = 8 * 1024 * 1024;
            ...
            LocalMemoryContext memoryContext)
    {
        ...
        this.stream = new ColumnChunkBufferedInputStream(requireNonNull(stream), Math.min(columnChunkSize, MAX_BUFFER_SIZE_DATA_SOURCE_READ));
            ...
            LocalMemoryContext memoryContext)
    {
        this.descriptor = requireNonNull(descriptor);
        this.stream = new ColumnChunkBufferedInputStream(requireNonNull(stream), Math.min(columnChunkSize, MAX_BUFFER_SIZE_DATA_SOURCE_READ));
        this.offsetIndex = requireNonNull(offsetIndex);
        this.memoryContext = requireNonNull(memoryContext, "ParquetColumnChunk memoryContext is null");
    }
            ...
            throws IOException
    {
        ...
        if (fileDecryptor.isPresent()) {
            ...
            if (headerBlockDecryptor != null) {
                dataPageHeaderAAD = AesCipher.createModuleAAD(fileDecryptor.get().getFileAAD(), ModuleType.DataPageHeader, rowGroupOrdinal, columnOrdinal, 0);
            }
        }
        ...
        final Iterator<DataPage> dataPageIterator = getDataPageIterator(dataPageHeaderAAD, headerBlockDecryptor, dictionaryPage);

        byte[] fileAAD = fileDecryptor.map(InternalFileDecryptor::getFileAAD).orElse(null);
        Optional<BlockCipher.Decryptor> dataDecryptor = getDataDecryptor(columnDecryptionSetup);

        long valueCountInColumnChunk = descriptor.getColumnChunkMetaData().getValueCount();
        return new PageReader(
                descriptor.getColumnChunkMetaData().getCodec(),
                dataPageIterator,
                valueCountInColumnChunk,
                dictionaryPage,
                offsetIndex,
                dataDecryptor,
                fileAAD,
    ...
    private Iterator<DataPage> getDataPageIterator(byte[] dataPageHeaderAAD, BlockCipher.Decryptor headerBlockDecryptor, DictionaryPage dictionaryPage)
    {
        return new Iterator<DataPage>()
        {
            ...
            public DataPage next()
            {
                ...
                try {
                    byte[] pageHeaderAdditionalAuthenticationData = dataPageHeaderAAD;
                    if (headerBlockDecryptor != null) {
    ...
    class ColumnChunkBufferedInputStream
