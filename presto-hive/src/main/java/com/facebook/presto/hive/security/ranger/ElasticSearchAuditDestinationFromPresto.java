...
public class ElasticSearchAuditDestinationFromPresto
        extends AuditDestination
{
    private static final Logger LOG = Logger.get(ElasticSearchAuditDestinationFromPresto.class);
    ...
    public ElasticSearchAuditDestinationFromPresto()
    {
    ...
    {
        if (client == null) {
            synchronized (ElasticSearchAuditDestinationFromPresto.class) {
                if (client == null) {
                    client = newClient();
                }
            }
    ...
    Map<String, Object> toDoc(AuthzAuditEvent auditEvent)
    {
        ...
        doc.put("evtTime", DEFAULT_DATE_PRINTER.print((auditEvent.getEventTime()).getTime()));
